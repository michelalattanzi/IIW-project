...
/* **********************************************************************************
 * leggi_comandi procedure: handles input command (file src, file dest, path) *
*************************************************************************************/

int leggi_comandi(struct cmd_s *cmd)
{
char str1[100];
	...
	//Inizialize mutex to global variable access
  	...
  	// configuration parameters and log file
   	...
   	if (l==1){
    	...
    	printf("IP remote server %s \n\n",inet_ntoa(srvaddr.sin_addr));

    	while (1){

        	fine=leggi_comandi(&com);
        	if (fine!=0) {
             		break;
		}
			...
			//file handling in source folder or creation in destination folder
				...
				strncpy(state->new_filename,com.path,DIM_NOMEFILE);
                	...
                	else if  (strcmp(com.istruz, "trm")==0)
			...
			socklen_t local_sinlen=(sizeof(state->local_sin)); //#include <sys/socket.h>
                	...
                	while(1) {
                        	...
                        	if(n==0){
                                	printf("\tProcess %d : the server is not responding, process ended \n", getpid());
                                	exit(EXIT_FAILURE);
                            	}
                	...
                	if (reply.type!=ACK){
				printf("\n\tProcess %d :  *********** Error: expected msg ACK - process ended\n",getpid());
                        	exit(EXIT_FAILURE);
                    	}
				...
				printf("\n\tProcess %d :   *********** Error: server encountered problems in command elaboration - process ended\n",getpid());
					...
					perror("Transmission failure: process ended \n");
                        	...
                        	ricevi(state); // receiving procedure on client
                    	}

                	else if (strcmp(com.istruz, "put")==0){
				trasmetti(state); // transmission procedure on client
                     	}
			close(state->s);
                	exit(0);
    	...
    	if (fclose(flog)!=0){
		perror("Error to close log file, process ended  \n");
         	exit(1);
      	}
