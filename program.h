...
struct config{
    ...
    int rit_send;      // transmission interval between two packets default value=0
...
int read_config(int argc, char *argv[],struct config *cfg){
    ...
    p = fgets(linea,max_line,fc); //tolto
...
void* time_out(void *args){
    ...
    if(((float)difftime(current.tv_sec, start_time.tv_sec))== cfg.RTO_in){
