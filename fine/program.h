...
// struct for retransmission timeout elaboration 
struct rtt_info {
...
// RTO calculation formula : TimeoutInterval = EstimateRTT + 4*DevRTT
...
void rtt_init(struct rtt_info *ptr){
	...
	//starting value of timeout management struct
	ptr->rtt_rtt    = 0;
	ptr->rtt_srtt   = 0;	
	ptr->rtt_rttvar = 0;		
	ptr->rtt_rto=RTO_in; 
...
/*************************************************************************************************
 * rtt_calc procedure: gets RTO value *
*************************************************************************************************/
...
float  rtt_calc(struct rtt_info *ptr, uint32_t ms){
	...
	// calculation with given formula
	ptr->rtt_rtt = ms ;
	delta = ptr->rtt_rtt - ptr->rtt_srtt;
	
	ptr->rtt_srtt += delta / 8;		 		/* alfa = 1/8 */
	
	if (delta < 0.0)
		delta = -delta;					/* |delta| */
	
	ptr->rtt_rttvar += (delta - ptr->rtt_rttvar) / 4;	/* beta = 1/4 */
...
void* time_out(void *args){
	...
	float init_timeout=params->duration; //RTO

	float local_timeout;
	params->frame->time_trasm=0;
	params->frame->ripet=1;      // to recognize repeated frames 
	...
	for (i=0; i< cfg.max_tent; i++){
		...
		usleep(local_timeout); //thread waits amount of time 
...
void* send_data(void *args){
    ...
    printf("\n\t Process %d - Transfer in progress: file/dir %s \n",getpid(),state->file_name);
  	...
  	while(1){
        ...
        int r = pthread_create(&(state->frame_buf[i].timeout), &tattr, time_out, &(state->frame_buf[i].timeout_state));
     	if (r!=0){
